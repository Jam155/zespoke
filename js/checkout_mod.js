...
jQuery(function($){
	...
	$checkout_form.wrap('<div  id="jcmc-tab-panels" class="jcmc-tab-wrapper row">');
	
	$('.jcmc-tab-wrapper form').addClass('ze-checkout-form-wrap small-12 medium-8 columns');
	...
	$('#jcmc-wrap').prepend('<div class="row ze-multicart-head"><div class="small-12 medium-6 columns nopadding"><h1 class="entry-title">Secure Checkout</h1></div><ul id="jcmc-tabs" class="jcmc-tabs ze-cart-multistep-wrapper small-12 medium-6 columns nopadding '+ style +'">' + tabs + '<li class="jcmc-even"><div><span class="jcmc-tab-span"><span class="jcmc-number">3</span>Complete</span></div></li></ul></div>');
	
	
	$('#jcmc-tab-panels').append('<div class="small-12 medium-4 columns checkout-col sidebar" id="cart-sidebar-wrapper">'+ '' +'</div>');
	
	var sidebar = wp.template('sidebar');
	$('#cart-sidebar-wrapper').html(sidebar());
	//console.log(sidebar());
	
	
	$('#jcmc-wrap').append('<div class="jcmc-buttons ' + button_style + '"><a href="#" class="jcmc-button jcmc-nextprev jcmc-prev">'+jcmc['text']['btn_prev']+'</a> <a href="#" class="jcmc-button jcmc-nextprev jcmc-next">'+jcmc['text']['btn_next']+'</a> <button type="button" class="jcmc-button jcmc-nextprev jcmc-order alt">'+jcmc['text']['btn_order']+'</a></div><div class="checkout-tnc">By clicking confirm order you are accepting our <a href="'+ ajaxurl +'/terms-and-conditions">Terms &amp; Conditions</a> of sale</div>');
	...
	$('#jcmc-trigger-0').on('click', function(event){
		$('#jcmc-trigger-1').removeClass('step-complete');
		$('#jcmc-trigger-0').removeClass('step-complete');
	});
	
	$('#jcmc-trigger-1').on('click', function(event){
		$('#jcmc-trigger-0').addClass('step-complete');
	});

	$('.jcmc-next').on('click', function(event) {

        	console.log("Anayltics Fired");

		if (typeof ga === "function") {
        		ga('send', { 'hitType': 'pageview', 'page': '/secure-checkout/payment-details', 'title': 'Payment Details'});
		}
	...
	$('.jcmc-nextprev').on('click', function(event){
		...
		if($(this).hasClass('jcmc-next')){
			...
			//jQuery('#sagepaydirect-card-name').val(name);
